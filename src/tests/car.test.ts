...
describe('Cars API', () => {
  ...
  beforeAll(async () => {
    const ownerRes = await request(app)
      .post('/auth/register')
      .send({ username: 'carowner', email: 'owner@example.com', password: 'password123' });
  ...
  describe('GET /cars', () => {
    beforeEach(async () => {
      await request(app).post('/cars').set('Authorization', `Bearer ${accessToken}`).send(carData);
    });
    ...
    it('should support filtering by make', async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.cars.every((c: { make: string }) => c.make === 'Tesla')).toBe(true);
    });
  });
  ...
  describe('POST /cars/:id/like', () => {
    ...
    it('should unlike a car listing when liked again (toggle)', async () => {
      ...
      await request(app).post(`/cars/${carId}/like`).set('Authorization', `Bearer ${accessToken}`);
      const res = await request(app)
        .post(`/cars/${carId}/like`)
        .set('Authorization', `Bearer ${accessToken}`);
      expect(res.status).toBe(200);
